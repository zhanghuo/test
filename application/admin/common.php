...
function show_operate($id, $operate = 'delete', $describe = '删 除', $controller = '')
{
    ...
    if($operate == 'delete'){
        $url = Url::build('admin/'.$controller.'/'.$operate);
        $ret = '<a class="label label-danger" onClick="check_delete(\''.$url.'\',\''.$id.'\');"><i class="fa fa-trash-o"></i>&nbsp;&nbsp;'.$describe.'</a>';
    }
    else{
        $url = Url::build('admin/'.$controller.'/'.$operate,['id'=>$id]);
        $ret = '<a class="label label-primary" href="'.$url.'"><i class="fa fa-pencil"></i>&nbsp;&nbsp;'.$describe.'</a>';
...
{
    $controller === '' && $controller = Request::instance()->controller();
    $url = Url::build('console/'.$controller.'/renewfield');
